...
const SHELL_TEMPLATE_FALLBACK = path.join(DIST_ROOT, DEFAULT_SHELL_DIST_SUBDIR, "index.html");
...
const createRequestHandler = (template: string, renderer: SsrRenderer | null) => {
...
const mountStaticIfPresent = async (app: Application, mountPath: string, directory: string) => {
...
export const setupProdFrontend = async (app: Application): Promise<FrontendLifecycle> => {
    ...
    const shellStaticMount = resolveMountPath(process.env.SHELL_STATIC_PATH, DEFAULT_SHELL_MOUNT);
