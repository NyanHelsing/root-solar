...
import { FlareButton, FlareCard, FlareStack, FlareTextInput } from "@root-solar/flare";
...
import { generateBeingKeyMaterial, type BeingKeyMaterial } from "../identities.ts";
import {
...
type WizardStage = "choose" | "login" | "register" | "download" | "verify" | "session";
...
export const BeingAccessWizard = <BeingRecord extends BeingRegistrationProfile>({
    ...
    const [stage, setStage] = useState<WizardStage>(() => (sessionRecord ? "session" : "choose"));
    const [status, setStatus] = useState<RegistrationStatus>("idle");
    const [error, setError] = useState<string | null>(null);
    const [name, setName] = useState("");
    const [snapshot, setSnapshot] = useState<BeingRegistrationSnapshot<BeingRecord> | null>(null);
    const [credentialPassword, setCredentialPassword] = useState("");
    const [credentialPasswordConfirm, setCredentialPasswordConfirm] = useState("");
    const [credentialError, setCredentialError] = useState<string | null>(null);
    const [credentialStatus, setCredentialStatus] = useState<"idle" | "creating" | "saved">("idle");
        ...
        return (
            <FlareStack gap="lg">
                ...
                <p className="rs-text-soft">
                    Sign in with an existing credential or create a new being to join the root.solar
                    commons.
                </p>
                <FlareStack direction="row" gap="lg" wrap>
                    ...
                    >
                        <FlareStack gap="md">
                            ...
                            <p className="rs-text-soft">
                                Import your credential file to unlock your keys and continue where
                                you left off.
                            </p>
                            <FlareButton
                                type="button"
                    ...
                    >
                        <FlareStack gap="md">
                            ...
                            <p className="rs-text-soft">
                                Generate signing and encryption keys, download a credential file,
                                then verify it to finish setup.
                            </p>
                            <FlareButton
                                type="button"
                                onClick={() => {
        ...
        <FlareStack gap="lg">
            ...
            {stage === "download" && snapshot ? (
                <FlareCard tone="muted" padding="lg">
                    <FlareStack gap="lg">
                        <p>
                            Set a passphrase and download the credential file. Store the file and
                            passphrase securely—you will need them to sign in.
                        </p>
                        <FlareStack gap="md">
                            ...
                            {credentialStatus === "saved" ? (
                                <p className="rs-text-success">
                                    Credential file saved. Keep it safe—you will verify it in the
                                    next step.
                                </p>
                            ) : null}
                            <FlareButton
                ...
                <FlareCard tone="muted" padding="lg">
                    <FlareStack gap="lg">
                        <FlareStack gap="sm">
                            ...
                            <p>
                                Welcome, <strong>{displaySession.name}</strong>. Your credential is
                                unlocked in this browser.
                            </p>
                            {!sessionPin ? (
                                <p className="rs-text-soft">
                                    Use the 4-digit session PIN you saved earlier. If it is
                                    misplaced, import your credential file again to generate a new
                                    session and PIN.
                                </p>
                            ) : null}
                        </FlareStack>
                        {sessionPin ? (
                            ...
                            >
                                ...
                                <span className="rs-text-soft">
                                    Keep the PIN handy while you work. If you forget it, re-import
                                    your credential file to start a new session.
                                </span>
                            </FlareStack>
                        ) : null}
