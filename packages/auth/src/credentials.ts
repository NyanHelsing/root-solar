...
import { encryptWithPassword, decryptToUtf8, type EncryptedPayload } from "./crypto/password.ts";
...
const decodeKeyPair = (payload: string): KeyPair => keyPairSchema.parse(JSON.parse(payload));
...
export const parseBeingCredentialFile = (input: unknown): BeingCredentialFile =>
    credentialSchema.parse(input);

export const parseBeingCredentialBundle = (input: unknown): BeingCredentialBundle =>
    credentialBundleSchema.parse(input);

export const deserializeBeingCredentialFile = (serialized: string): BeingCredentialFile =>
    parseBeingCredentialFile(JSON.parse(serialized));
