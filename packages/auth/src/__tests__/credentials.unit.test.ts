...
describe("auth/credentials", () => {
  ...
  it("creates, serializes, parses, and decrypts credential files", async () => {
    ...
    const credentialFile = await credentials.createBeingCredentialFile(
      bundle,
      "secret",
    );
    ...
    assert.equal(credentialFile.signing.encryptedKeyPair.iterations, credentialFile.encryption.encryptedKeyPair.iterations);
    ...
    const decrypted = await credentials.decryptBeingCredentialFile(
      credentialFile,
      "secret",
    );
  ...
  it("handles file reader failures", async () => {
    ...
    await assert.rejects(
      credentials.readFileAsText(new FakeFile("reject.json")),
      /boom/,
    );
