...
export const router = t.router({
    listAxioms: procedure
        ...
        .query(({ ctx, input }) => ctx.missives.list({ sentimentSlug: input?.sentiment })),
    listTags: procedure.output(listTagsOutput).query(({ ctx }) => ctx.tags.list()),
    getAxiom: procedure.input(getAxiomInput).query(async ({ input, ctx }) => {
    ...
    addMissiveTag: procedure.input(addMissiveTagInput).mutation(async ({ input, ctx }) => {
    ...
    addTagTag: procedure.input(addTagTagInput).mutation(async ({ input, ctx }) => {
    ...
    removeMissiveTag: procedure.input(removeMissiveTagInput).mutation(async ({ input, ctx }) => {
    ...
    setSentiment: procedure.input(sentimentInput).mutation(({ input, ctx }) =>
    ...
    startBeingRegistration: procedure
        ...
        .mutation(({ ctx, input }) => createBeingRegistrationResolver(ctx).start(input)),
