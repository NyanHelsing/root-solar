...
describe("api/router", () => {
  it("routes requests through the context models", async () => {
    ...
    const removedMissiveTags = [];
    const upsertedSentiments = [];
    const removedSentiments = [];
    const createdComments = [];
    const listedSentimentsArgs = [];
    const ctx = {
      missives: {
        async list(params) {
          return [
        ...
        async addTag(input) {
          ...
          return {
            ...
            tags: [{ id: `tag:${input.tagSlug}`, slug: input.tagSlug, label: input.tagSlug }],
          };
        },
      ...
      tags: {
        ...
        async addParent(input) {
          ...
          const normalizedId = input.tagId.includes(":") ? input.tagId : `tag:${input.tagId}`;
          const slug = normalizedId.includes(":") ? normalizedId.slice(normalizedId.indexOf(":") + 1) : normalizedId;
          return {
            id: normalizedId,
            slug,
            label: slug,
    ...
    assert.equal(ensuredTagDefinitions.at(-1)?.slug, "meta");
    ...
    const lastSentimentArgs = listedSentimentsArgs.at(-1);
