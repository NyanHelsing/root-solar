...
import { RecordId, type StringRecordId } from "surrealdb";
...
describe("axiom entity", () => {
    it("lists axioms sorted and filtered by sentiment", async () => {
        ...
        assert.deepEqual(
            filtered.map((record) => record.title),
            ["Alpha", "Beta"],
        );
    });
    ...
    it("retries axiom creation on duplicate ids", async () => {
        ...
        const { ctx } = createTestContext({
            ...
            tags: {
                ensureMany: async (definitions: Array<{ slug: string; label: string }>) => {
    ...
    it("throws when the database returns no record", async () => {
        ...
        const { createAxiomModel } = await import("../entity.ts");
        const model = createAxiomModel(ctx);
        await assert.rejects(model.create({ title: "Broken" }), /returned empty record/);
    });
    ...
    it("normalises tag inputs when appending to a missive", async () => {
        ...
        const mergePayloads: Array<{ id: unknown; value: Record<string, unknown> }> = [];
        let selectCount = 0;
        ...
        const { ctx } = createTestContext({
            db: {
                ...
                merge: async (id: unknown, value: Record<string, unknown>) => {
                    mergePayloads.push({ id, value });
                    return value;
                },
            } as unknown as Context["db"],
            tags: {
    ...
    it("removes tags from a missive", async () => {
        ...
        const { ctx } = createTestContext({
            db: {
                ...
                merge: async (
                    _id: StringRecordId | RecordId | string,
                    payload: Record<string, unknown>,
                ) => {
                    mergePayloads.push(payload);
                    if (Array.isArray(payload?.tags)) {
                        storedTags = payload.tags as string[];
                    }
                    return payload;
                },
            } as unknown as Context["db"],
        ...
        const record = await model.removeTag({ missiveId: "missive:1", tagSlug: "tag:new" });
        ...
        assert.deepEqual(mergePayloads[0], { tags: ["tag:existing"] });
    });
});
