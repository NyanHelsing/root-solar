...
const selectMany = async (ctx: Context, ids: string[]) => {
  ...
  const selections = await Promise.all(
    unique.map(async (identifier) => {
      const record = await ctx.db.select<RawTagRecord>(
        new StringRecordId(identifier),
      );
...
export const createTagModel = (ctx: Context) => {
  return {
    ...
    async upsert({
      ...
      const record = await ctx.db.upsert<RawTagRecord>(
        recordId,
        {
          id: recordId,
          slug,
    ...
    async ensureMany(definitions: Array<{
      slug: string;
      label: string;
      description?: string;
      tags?: string[];
