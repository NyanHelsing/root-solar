...
describe("tag entity", () => {
  it("lists tags with stable ordering", async () => {
    ...
    assert.deepEqual(tags.map((tag) => tag.slug), ["alpha", "beta"]);
  });
  ...
  it("fetches multiple tags by id list", async () => {
    const records = new Map<string, { id: RecordId | StringRecordId; slug: string; label: string }>([
      [
        "tag:one",
        {
    ...
    assert.deepEqual(many.map((tag) => tag.id), ["tag:one"]);
  ...
  it("adds parent tags while ensuring definitions", async () => {
    ...
    const { ctx } = (() => {
      const baseCtx = createTestContext({
        db: {
          ...
          merge: async (identifier: RecordId | StringRecordId, payload: { tags: string[] }) => {
    ...
    assert.ok(updated);
    assert.equal(updated?.tags?.includes("tag:parent-tag"), true);

    const storedChild = storedRecords.get("tag:child");
    assert.ok(storedChild);
    assert.deepEqual(storedChild?.tags, ["tag:parent-tag"]);

    const noDuplicate = await model.addParent({ tagId: "child", parentTagSlug: "tag:parent-tag" });
    assert.ok(noDuplicate);
    assert.equal(noDuplicate?.tags?.filter((slug) => slug === "tag:parent-tag").length, 1);
  });
});
