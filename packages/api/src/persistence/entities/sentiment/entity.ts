...
type RawSentimentRecord = Omit<SentimentRecord, "id" | "beingId" | "subjectId"> & {
    id: string | RecordId;
...
export const createSentimentModel = (ctx: Context) => {
    return {
        ...
        async listForBeing(beingId: string, options?: { tagId?: string; subjectTable?: string }) {
            ...
            const allocations = hydrated.map((sentiment) => {
                ...
                const ratio = totalWeightForTag === 0 ? 0 : sentiment.weight / totalWeightForTag;
                return {
                    ...sentiment,
                    totalWeightForTag,
                    ratio,
