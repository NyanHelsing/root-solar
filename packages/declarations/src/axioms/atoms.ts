...
export const loadAxiomsAtom = atom(null, async (get, set) => {
  ...
  try {
    ...
    const order = axioms.map((record) => {
      const recordWithKind: MissiveRecord = { ...record };
      ...
      const base: AxiomEntity = existing
        ? {
            ...
            record: {
              ...existing.record,
              ...recordWithKind,
              kind: recordWithKind.kind ?? existing.record.kind,
            },
          }
        : {
            record: recordWithKind,
            sentiments: {},
          };
...
export const loadAxiomDetailAtom = atom(null, async (get, set, axiomId: string) => {
  ...
  try {
    ...
    const detailRecord: MissiveRecord = { ...detail };

    const existing = latest.entities[axiomId];
    const entity: AxiomEntity = existing
      ? cloneEntity(existing)
      : {
          record: detailRecord,
          sentiments: {},
    ...
    entity.record = {
      ...
      kind: detailRecord.kind ?? entity.record.kind,
